...
- **Active chat row: persistent highlight, no accent color** (issue #72) — the current chat in the sidebar carries the same background highlight as hover, permanently, instead of the blue accent text and filled bullet dot; the row looks identical whether the pointer is over it or not, and the blue tint is gone from the active row entirely. The highlight also stays while the agent is running commands in that chat — previously every command run visually unfocused the current row (only the spinner remained). Chat titles are one step larger (13px), and the working-chat spinner is neutral muted grey instead of accent blue. Project group names render in grey one step lighter than the Projects/Chats section labels, lifting to full text color on hover.
