...
- **New chat appears in the sidebar immediately** (issue #55) — clicking New Chat (nav item, project `+`, or Chats header `+`) shows the chat in the left sidebar right away with a "New chat" placeholder title, under its project group or in Chats, instead of it being invisible until the first message creates the session. The placeholder row updates to the first-message title as soon as you send one, and is replaced by the real session row after the sync — no duplicates, no empty sessions left in the database when you close a blank chat. Clicking the row switches back to that chat.
