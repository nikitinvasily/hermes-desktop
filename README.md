...
- **Project picker for the session working folder** (issue #29) — the context-folder chip in the chat input lists your projects instead of recent raw paths: picking a project sets its primary directory as the session's working folder. Projects without a primary directory are disabled, and "Open folder..." remains for arbitrary paths (native dialog locally, in-app browser over SSH/remote).
