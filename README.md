...
- **Complete "Move to project" picker + agent-side "New folder…"** (issues #36, #37) — the session menu's project picker lists every loaded agent project (zero-session ones included), not just folders derived from the visible session window, which over SSH used to hide projects behind cwd-less cron rows. "New folder…" in remote/SSH modes opens a directory browser over the agent host's filesystem (starting at its workspace) instead of the native macOS picker, whose local path was meaningless for a remote agent.
