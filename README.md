...
- **Session archiving** (issue #34) — a chat's context menu gains an Archive action that hides it from the sidebar list without deleting anything, and a collapsible Archive section at the bottom of the sidebar lists the current connection's archived chats with restore and delete actions. Archiving flips the agent's own native `archived` flag (local state.db / dashboard REST API over remote and SSH), so the CLI and dashboard see the same state; archiving the open chat leaves it open, only the list entry disappears.
