...
- **Remote connection loss is loud, not silent** — when the remote agent's dashboard stops answering (expired session or lost network), the sidebar drops its stale chats/projects instead of showing half-loaded data, data screens dim and block input, and a banner at the bottom of the sidebar offers a one-click sign-in (expired session) or retry (network). As soon as the connection answers again, chats and projects reload from the server with your sidebar sections kept as you left them (collapsed stays collapsed).
...
- **Chats keep their photos and replies after context compaction** — long-running sessions on the agent used to silently lose earlier assistant answers (together with generated images) once the agent compacted its context; duplicated service blocks also piled up in the transcript. The full history is now loaded again, duplicates are collapsed, and the compaction note shows as a small collapsed card.
