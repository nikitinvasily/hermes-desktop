...
- **Subagents panel in the chat** — when the agent delegated work in a chat, a small floating panel appears top-right over the conversation listing every subagent run (its goal, a spinner while it works). Click a subagent to read its full transcript. The panel only exists in chats that actually used subagents.
