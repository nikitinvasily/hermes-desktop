...
### Pending sidebar rows

A brand-new chat is visible in the sidebar immediately, before any session exists (issue #55): no database row is created, the row is an ephemeral renderer-side entry derived from the run itself.

[[src/renderer/src/screens/Layout/chatRuns.ts#pendingSidebarRows]] maps every sessionless run of the active connection+profile to a `pending-<runId>` row titled `run.title` (the first user message, once sent) with the localized "New chat" fallback, carrying `initialContextFolder` as its grouping folder. Runs that already reported a session id are excluded — their real row comes from the sessions cache, and emitting both would duplicate the chat. [[src/renderer/src/screens/Layout/Layout.tsx#Layout]] passes the rows plus `activePendingRunId` (the active run's id when its session is still null) into [[src/renderer/src/screens/Layout/SidebarRecentSessions.tsx]], which prepends them to their project group (creating the group when the folder has none yet) or to the flat Chats list, ahead of the recency-ordered cached rows. A pending row renders without the options button and context menu (there is no session behind it to rename/archive/move/delete), highlights via `activePendingRunId`, and clicking it activates the run through `onOpenPendingRun` → `handleActivateRun`.

The row disappears by itself when the run's `sessionId` lands AND the sidebar's synced list contains that session — `pendingSidebarRows` receives the synced ids (`SIDEBAR_SYNCED_IDS_EVENT`, fired by the sidebar on every list load) and holds the row across the gap between the session-id announcement (mid-send) and the cache sync painting the real row; dropping it at the announcement alone made the chat vanish from the sidebar until the next refresh. Closing the run also removes the row. This composes with provisional fresh sessions (below): a failed first turn leaves no sidebar residue beyond the still-pending row, which stays until the chat is used or closed. [[src/renderer/src/screens/Layout/chatRuns.test.ts]] covers the row derivation (connection/profile/session filters, title fallback) and the hold-until-synced release; [[src/renderer/src/screens/Layout/SidebarRecentSessions.test.tsx]] covers placement, activation click, and the missing options button.
