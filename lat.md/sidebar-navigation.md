...
The row disappears by itself when the run gains a `sessionId` (the existing `currentSessionId`-change refresh then paints the real session with its generated title) or when the run is closed. This composes with provisional fresh sessions (below): a failed first turn leaves no sidebar residue beyond the still-pending row, which stays until the chat is used or closed. [[src/renderer/src/screens/Layout/chatRuns.test.ts]] covers the row derivation (connection/profile/session filters, title fallback); [[src/renderer/src/screens/Layout/SidebarRecentSessions.test.tsx]] covers placement, activation click, and the missing options button.
