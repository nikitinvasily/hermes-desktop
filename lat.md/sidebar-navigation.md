...
Because the agent never writes the watermark on its own paths, two guards keep the flag meaningful: [[src/main/session-cache.ts#syncSessionCache]] runs a one-time baseline (tracked as `readBaselineDone` in the desktop cache file, per profile) stamping NULL rows with their own last activity, so a fresh install of the feature does not flip every historical session to unread; and a finished turn in the VISIBLE chat stamps the watermark immediately ([[src/renderer/src/screens/Layout/Layout.tsx]]'s `handleRunLoading` marks the active run's session read), so only background completions light the unread dot.
