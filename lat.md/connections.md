...
SSH connection creation is hidden behind [[src/renderer/src/utils/ssh-ui-flag.ts#SSH_UI_ENABLED]] (`false` during the remote-parity transition): the Welcome screen's "Connect via SSH" buttons and the Settings connection-mode SSH option disappear, while an ALREADY-saved SSH connection keeps its row in the switcher, stays selectable, and re-shows the SSH mode/editor while active (existing setups are never broken; flip the flag to re-expose creation). The server's `HERMES_HOME` is resolved once via [[src/main/remote-metadata.ts#remoteGetHermesHome]] and cached (TTL 5 minutes), invalidated by [[src/main/remote-settings.ts#remoteInvalidateSettingsCaches]] on every connection switch.
