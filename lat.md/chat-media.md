...
Incoming photos end with a vision hint (`[… use vision_analyze with image_url: <path> ~]`) — the `~]` suffix defeats the generic inline bare-path matcher, so a dedicated rule extracts the path as a trusted media-token (the gateway just wrote the file). The bracketed description block itself (`[The user sent an image~ …]`) becomes a `vision-note` segment rendered by [[src/renderer/src/components/MediaImage.tsx#VisionNote]] as a collapsible "what the agent saw" card — collapsed by default, the image is the primary content. Voice notes arrive as `[The user sent a voice message: <path> (duration: …)]` or the untranscribed variant and extract the same way, flagged as voice messages. Incoming documents (`[The user sent a document: 'x.json'. It is saved at: <path> …]`, paths may contain spaces) extract as document tokens that keep the user's original filename and render through [[src/renderer/src/components/MediaImage.tsx#DocumentCard]] as a file card. Because user bubbles are otherwise verbatim text, [[src/renderer/src/screens/Chat/mediaUtils.ts#hasUserMediaHints]] cheaply gates which user rows run the media pipeline at all ([[src/renderer/src/screens/Chat/MessageRow.tsx#MessageRow]]); hints inside fenced/inline code are ignored.

## Deleted-file distinction

When a media read fails, a follow-up existence probe separates "file no longer exists" from a load error.

Server-side cleanup is real: TTS payloads written to `/tmp` vanish (PrivateTmp, see below), gateway cache entries get rotated. [[src/renderer/src/components/MediaImage.tsx#MediaImage]], [[src/renderer/src/components/MediaImage.tsx#AudioPlayer]] and [[src/renderer/src/components/MediaImage.tsx#DocumentCard]] therefore probe `mediaFileExists` after a failed read: file verified missing → "file is no longer available on the agent" (`chat.media.fileGone`); file present but unreadable → the generic load-error message. Specified by the DocumentCard tests in [[src/renderer/src/components/MediaImage.document.test.tsx]].
